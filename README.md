...
This repo currently provides a working MCP/skill core. It is ready for AI-hosted conversations and demos, but it is **not yet** a standalone SaaS app, custom chat UI, live web researcher, or meal-kit platform.

What works now:

- a host AI can call the MCP server through stdio;
- the server executes the full MCP workflow from food-memory fragment to recipe-generation handoff;
- research facts supplied by a host AI can be converted into typed provenance records;
- the final recipe is generated by the host AI using the structured context, not by deterministic recipe code inside the MCP server.
