...
describe('defaultCachePath', () => {
  const originalEnv = process.env.MEMBER_BERRIES_CACHE_PATH;

  afterEach(() => {
    if (originalEnv !== undefined) {
      process.env.MEMBER_BERRIES_CACHE_PATH = originalEnv;
    } else {
  ...
  it('rejects non-.db file extensions', () => {
    process.env.MEMBER_BERRIES_CACHE_PATH = '/tmp/cache.sqlite';
    expect(() => defaultCachePath()).toThrow(/must end with \.db/);
  });

  it('rejects bare directory path without extension', () => {
    process.env.MEMBER_BERRIES_CACHE_PATH = '/tmp/some-directory';
    expect(() => defaultCachePath()).toThrow(/must end with \.db/);
  });

  it('accepts valid .db path', () => {
    process.env.MEMBER_BERRIES_CACHE_PATH = '/tmp/test-cache.db';
  ...
  it('returns XDG default when env var is unset', () => {
    ...
    expect(result).toContain('member-berries');
