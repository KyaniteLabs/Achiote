...
describe('package distribution metadata', () => {
  it('declares install, type, executable, and source-control metadata', () => {
    ...
    expect(pkg.bin).toEqual({ 'member-berries': 'dist/index.js' });
  ...
  it('publishes only intentional package files', () => {
    expect(pkg.files).toEqual(['dist/', 'skill/', 'docs/ARCHITECTURE.md', 'docs/ROADMAP.md', 'README.md', 'LICENSE']);
  });
  ...
  it('has reproducible quality and packaging scripts', () => {
    ...
    expect(pkg.scripts['pack:check']).toBe('npm run check && npm pack --dry-run');
  });
});
