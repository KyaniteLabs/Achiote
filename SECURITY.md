...
- **Authentication** — configurable via `ACHIOTE_AUTH_ENABLED` env var. When enabled, requires API key via `x-api-key` header or `Authorization: Bearer` header. Keys are tiered (free/pro/business/enterprise) and should be stored as `keyHash` records generated by `npm run keygen`, not as plaintext raw keys. Anonymous `/ask` requires the separate local-demo-only `ACHIOTE_ALLOW_ANON_ASK=true` opt-in when auth is disabled.
