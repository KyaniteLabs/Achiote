...
const transcript = {
  ...
  expectText: [/sour|acid|lime|vinegar|ferment/i, /herb|herby|green/i, /question|ask|country|region|where/i],
  expectEvidence: [/abuela/i],
  expectReceipt: [/Achiote Memory Receipt/i, /userSaid|User-Said|User-said/i, /inferred|Inferred/i, /unknown|Unknown/i],
};
...
async function runTranscript() {
  ...
  try {
    ...
    const missingEvidence = transcript.expectEvidence.filter((pattern) => !pattern.test(combined));
    ...
    const pass = response.ok && done && receiptEvents.length > 0 && missingText.length === 0 && missingEvidence.length === 0 && missingReceipt.length === 0 && forbidden.length === 0;
    ...
    return {
      ...
      missingEvidence: missingEvidence.map(String),
