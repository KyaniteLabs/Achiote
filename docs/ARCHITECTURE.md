...
Local model runs also record the load configuration as first-class telemetry. `src/lib/local-inference-profiles.ts` defines `speed`, `quality`, and `memory` profiles with explicit LM Studio native REST load payloads plus advanced SDK-only knobs that should remain visible even when the REST endpoint cannot send them. It also records LM Studio endpoint families separately: OpenAI-compatible chat completions (`/v1/chat/completions`), OpenAI Responses (`/v1/responses`), legacy completions (`/v1/completions`), Anthropic-compatible Messages (`/v1/messages`), and native stateful chat (`/api/v1/chat`). Achiote runtime currently supports OpenAI chat-completions and Anthropic Messages; Responses and native chat are profiler/diagnostic surfaces until dedicated adapters exist.

`scripts/local-inference-profiler.mjs` uses LM Studio `/api/v1/models/load` for load telemetry and direct probe routes for endpoint-style experiments, preserving intended load payloads, echoed load config, usage, latency, errors, text previews, and reasoning-trace previews when a model exposes them. REST-applied settings such as context length, eval batch, parallelism, Flash Attention, GPU KV offload, and MoE experts are separated from SDK/CLI-required settings such as CPU thread count, KV cache quantization, keep-warm behavior, and GPU offload policy.

Provider compatibility is modeled at provider/model granularity. `resolveProviderCapabilityProfile` keeps provider kind, model, endpoint style, base URL, native-tool posture, timeout posture, and rate-limit sensitivity together so `/ask` routing and QA telemetry describe the same runtime truth. GLM/Z.ai is treated as a provider family, not one endpoint: current GLM 5.x and 4.7 Coding Plan models default to Anthropic-compatible routing, while older GLM 4.5 Air/Flash defaults to OpenAI-compatible Coding Plan routing unless explicitly overridden for comparison. OpenRouter also stays model-specific: `:free` models are marked rate-limit sensitive, and catalog metadata is used when available before assuming native tool or `tool_choice` support. Weak-cloud OpenRouter rows keep selected catalog capability metadata, including supported parameters and context length, so the telemetry miner can separate model capability gaps from provider transport failures.
