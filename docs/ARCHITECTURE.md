...
GLM/Z.ai is also treated as a provider family, not one endpoint. Coding Plan defaults stay on Anthropic-compatible routing for current models, but the weak-cloud matrix records `endpointStyle` so older GLM 4.5-family models can be compared through both Anthropic-compatible and OpenAI-compatible Coding Plan routes.
